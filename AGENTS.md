...
`pdf-full10k-after-scs-europe-a83f26c`: 2,409/6,293 `good_pdf` (38.28%), gap
3,570 rows, with one `missing_pdf_harvest -> good_pdf` transition and zero
good-to-non-good regressions. This is bounded direct-PDF cache/reharvest plus
read-only full-gate measurement, not a Taxicab-main production scraping push.

Latest #461 report publish: oxjobs commit `06d7888e4` publishes the SCS Europe
accepted recovery, full gate, residual refresh
`residual-clusters-after-scs-europe-a83f26c`, 0-100 anchored bar chart, learning
notes, improvement plan, and next `sciencepubco.com` handoff. CI run `27859307575`
