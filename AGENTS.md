...
Latest completed batch: batch 010, output in `batch_e2e_runs/batch-010/`.
Result: 100 checked, 76 ready rows, 24 review rows, 69 passes, 7 failures,
90.79% on ready rows, 29/29 public-PDF rows retrieved by Taxicab, and 84 rows
with useful Parseland output. All seven failures are label mismatches where the
sidecar said no public PDF but Taxicab found a real PDF.

Cumulative batch loop result through batches 001-010: 1,000 checked, 679 ready
rows, 321 review rows, 633 passes, 46 failures, 93.23% on ready rows, 245/246
public-PDF rows retrieved by Taxicab, and 850 rows with useful Parseland output.
...
```bash
python3 scripts/taxicab_batch_e2e.py --batch-number 11 --batch-size 100 --out batch_e2e_runs --workers 4 --timeout 90 --reharvest
```
