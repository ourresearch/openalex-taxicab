...
Latest completed batch: batch 036, output in `batch_e2e_runs/batch-036/`.
Result: 100 checked, 71 ready rows, 29 review rows, 65 passes, 6 failures,
91.55% on ready rows, 23/23 public-PDF rows retrieved by Taxicab, and 79 rows
with useful Parseland output. Batch 036 added no public-PDF misses. All six
scored failures are label mismatches where Taxicab found a real PDF even though
the sidecar says no public PDF.

Cumulative batch loop result through batches 001-036: 3,600 checked, 2,454 ready
rows, 1,146 review rows, 2,267 passes, 187 failures, 92.38% on ready rows,
909/914 public-PDF rows retrieved by Taxicab, and 3,062 rows with useful
Parseland output. The five public-PDF misses remain the original ScienceDirect/JMRT PDF-byte retrieval issue from batch 006, the batch 018 OSF label-review case where the sidecar URL downloads DOCX rather than PDF, the batch 027 revistas.filos.unam.mx tiny/corrupt PDF-like response, one batch 031 Taxicab DOI-lookup failure, and the batch 034 revistas.uach.cl tiny/corrupt PDF-like response. Batch 036 added no public-PDF misses and six label mismatches where Taxicab found a real PDF even though the sidecar says no public PDF.

Next command:

```bash
python3 scripts/taxicab_batch_e2e.py --batch-number 37 --batch-size 100 --out batch_e2e_runs --workers 4 --timeout 90 --reharvest
```
