...
Latest completed batch: batch 094, output in `batch_e2e_runs/batch-094/`.

```text
batch 094: 100 checked, 68 ready rows, 32 review rows, 59 passes, 9 failures,
86.76% on ready rows, 13/13 public-PDF rows retrieved by Taxicab,
26 real PDFs found by Taxicab, 79 useful Taxicab HTML rows, and 81 rows with
useful Parseland output.
```

Batch 094 added no public-PDF miss. Its nine scored failures are label mismatches
where Taxicab found real PDFs even though the sidecar says no public PDF. Hosts:
`di.aerzteblatt.de` (1), `doi.org` (2), `ijprajournal.com` (1),
`indjaerospacemed.com` (1), `nature.com` (1),
`s3.ca-central-1.amazonaws.com` (1), `sciencedirect.com` (1), and
`worldscientific.com` (1).

Cumulative batch loop result through batches 001-094:

```text
9,400 checked, 6,510 ready rows, 2,890 review rows, 6,074 passes,
436 failures, 93.30% on ready rows, 2,349/2,357 public-PDF rows retrieved by
Taxicab, 2,930 real PDFs found by Taxicab, 7,371 useful Taxicab HTML rows,
and 8,049 rows with useful Parseland output.
```
...
```bash
python3 scripts/taxicab_batch_e2e.py --batch-number 95 --batch-size 100 --out batch_e2e_runs --workers 2 --timeout 90 --reharvest
```
