...
Latest completed batch: batch 011, output in `batch_e2e_runs/batch-011/`.
Result: 100 checked, 68 ready rows, 32 review rows, 66 passes, 2 failures,
97.06% on ready rows, 28/28 public-PDF rows retrieved by Taxicab, and 88 rows
...
Cumulative batch loop result through batches 001-011: 1,100 checked, 747 ready
rows, 353 review rows, 699 passes, 48 failures, 93.57% on ready rows, 273/274
public-PDF rows retrieved by Taxicab, and 938 rows with useful Parseland output.
...
```bash
python3 scripts/taxicab_batch_e2e.py --batch-number 12 --batch-size 100 --out batch_e2e_runs --workers 4 --timeout 90 --reharvest
```
