...
Latest completed batch: batch 039, output in `batch_e2e_runs/batch-039/`.
Result: 100 checked, 71 ready rows, 29 review rows, 68 passes, 3 failures,
95.77% on ready rows, 25/25 public-PDF rows retrieved by Taxicab, and 88 rows
with useful Parseland output. Batch 039 added no public-PDF misses. All three
scored failures are label mismatches where Taxicab found a real PDF even though
the sidecar says no public PDF.

Cumulative batch loop result through batches 001-039: 3,900 checked, 2,657 ready
rows, 1,243 review rows, 2,455 passes, 202 failures, 92.40% on ready rows,
975/980 public-PDF rows retrieved by Taxicab, and 3,328 rows with useful
Parseland output. The five public-PDF misses remain the original ScienceDirect/JMRT PDF-byte retrieval issue from batch 006, the batch 018 OSF label-review case where the sidecar URL downloads DOCX rather than PDF, the batch 027 revistas.filos.unam.mx tiny/corrupt PDF-like response, one batch 031 Taxicab DOI-lookup failure, and the batch 034 revistas.uach.cl tiny/corrupt PDF-like response. Batch 039 added no public-PDF misses and three label mismatches where Taxicab found a real PDF even though the sidecar says no public PDF.

Next command:

```bash
python3 scripts/taxicab_batch_e2e.py --batch-number 40 --batch-size 100 --out batch_e2e_runs --workers 4 --timeout 90 --reharvest
```
