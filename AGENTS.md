...
Latest completed batch: batch 056, output in `batch_e2e_runs/batch-056/`.
Result: 100 checked, 69 ready rows, 31 review rows, 65 passes, 4 failures,
94.20% on ready rows, 25/25 public-PDF rows retrieved by Taxicab, and 91 rows
with useful Parseland output. Batch 056 added no public-PDF miss. Its four
failures are label mismatches on `antidiskriminierungsstelle.de`, `mdpi.com`,
`proceedings.com`, and `psycnet.apa.org`, where Taxicab found a real PDF even
though the sidecar says no public PDF.

Cumulative batch loop result through batches 001-056: 5,600 checked, 3,844 ready
rows, 1,756 review rows, 3,571 passes, 273 failures, 92.90% on ready rows,
1,422/1,427 public-PDF rows retrieved by Taxicab, 1,768 real PDFs found by
Taxicab, and 4,803 rows with useful Parseland output. The same six public-PDF attention rows are unchanged: the original ScienceDirect/JMRT PDF-byte retrieval issue from batch 006, the batch 018 OSF label-review case where the sidecar URL downloads DOCX rather than PDF, the batch 027 revistas.filos.unam.mx tiny/corrupt PDF-like response, one batch 031 Taxicab DOI-lookup review row, the batch 034 revistas.uach.cl tiny/corrupt PDF-like response, and the batch 045 IJST/SciResOL S3 public-PDF miss.

Next command:

```bash
python3 scripts/taxicab_batch_e2e.py --batch-number 57 --batch-size 100 --out batch_e2e_runs --workers 4 --timeout 90 --reharvest
```
