...
Latest completed batch: batch 004, output in `batch_e2e_runs/batch-004/`.
Result: 100 checked, 65 ready rows, 35 review rows, 60 passes, 5 failures,
92.31% on ready rows, 25/25 public-PDF rows retrieved by Taxicab, and 82 rows
with useful Parseland output. The five failures are label mismatches where the
sidecar said no public PDF but Taxicab found a real PDF.

Cumulative batch loop result through batches 001-004: 400 checked, 272 ready
rows, 128 review rows, 250 passes, 22 failures, 91.91% on ready rows, 102/102
public-PDF rows retrieved by Taxicab, and 340 rows with useful Parseland output.
All 22 failures are label mismatches; there are no public-PDF misses yet.

Next command:

```bash
python3 scripts/taxicab_batch_e2e.py --batch-number 5 --batch-size 100 --out batch_e2e_runs --workers 4 --timeout 90 --reharvest
```
