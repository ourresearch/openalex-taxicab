...
Latest IOP Science REVIEW evidence slice: private exact-host no-storage Zyte run `iopscience-review-pack-provider5-d36471a` sampled `5` `iopscience.iop.org` rows from the private top-250 REVIEW pack. It recovered valid PDF bytes for `2/5` rows; residual best categories were `interstitial_or_paywall=2` and `bot_block_403=1`. Private overlay joined all five rows and would move `2` rows from REVIEW to public TRUE/all-known TRUE, `2` rows from REVIEW to public FALSE/all-known TRUE, and leave `1` row REVIEW. Public aggregate asset `evidence/report461-iopscience-review-provider-overlay-summary-d36471a.json` is published in oxjobs #461 at commit `725f3c0bf`; CI run `28068959982` passed and the live raw report/asset were verified after cache retry. Accepted sidecar counts are unchanged until the private overlay is reviewed and adopted.

Recent REVIEW evidence, newest first: `aip.scitation.org` stayed REVIEW (`0/5`, `html_instead_of_pdf=3`, `js_redirect_unresolved=2`); `journals.sagepub.com` stayed REVIEW (`0/5`, `js_redirect_unresolved=4`, `empty_response=1`); `journals.uchicago.edu` found one public TRUE candidate (`1/5`, residual `js_redirect_unresolved=4`); `brill.com` stayed REVIEW (`bot_block_403=5`); `opg.optica.org` found one public TRUE candidate. Older no-movement samples remain REVIEW unless a private overlay is explicitly reviewed and adopted.

Review-pack top hosts already sampled for denominator evidence include `api.taylorfrancis.com`, `jstor.org`, `pubs.acs.org`, `degruyterbrill.com`, `journals.lww.com`, `spiedigitallibrary.org`, `thieme-connect.de`, `pubs.rsc.org`, `tandfonline.com`, `pubs.aip.org`, `link.aps.org`, `doi.org`, `opg.optica.org`, `brill.com`, `journals.uchicago.edu`, `journals.sagepub.com`, `aip.scitation.org`, and `iopscience.iop.org`.

Good next unsampled review-pack hosts include `thelancet.com`, `nature.com`, `asmedigitalcollection.asme.org`, `jamanetwork.com`, `karger.com`, `sk.sagepub.com`, `pdcnet.org`, `actahort.org`, `eurekaselect.com`, and `journals.ametsoc.org`. Higher-volume hosts such as `link.springer.com`, `onlinelibrary.wiley.com`, `sciencedirect.com`, `cambridge.org`, `academic.oup.com`, and `papers.ssrn.com` have substantial prior evidence elsewhere; do not rerun them unless testing a new provider-advised recipe or a specific denominator question.
...
```bash
cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab && python3 scripts/provider_pdf_probe.py --input /Users/shubh-trips/Documents/OpenAlex/parseland-eval/eval/data/merged-FINAL-pdf-review-pack-top250.csv --category '' --host thelancet.com --limit 5 --strategies all --out /tmp/taxicab-pdf-review-provider-probes --run-id thelancet-review-pack-provider5-<taxicab-sha> --env-file .env --timeout 45 --sleep 0.5
```
