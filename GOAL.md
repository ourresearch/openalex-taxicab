...
## Current PDF Handoff: 2026-06-23 Accepted Public TRUE Cache Full Gate

Taxicab PDF Phase 2 eval/reporting work is merged to Taxicab `main` at `8b36486`. The latest accepted full 10K read-only gate is `taxicab-pdf-after-publictrue-cache-477d2f8`: `2,457/6,293 good_pdf` (`39.04%`) on the legacy guessed-PDF denominator, `+12` versus the main-merge gate and `+620` versus the first measured denominator reference of `1,837/6,293` (`29.19%`). The legacy raw 95% target is `5,979/6,293`, so the raw-denominator gap is `3,522` rows. The accepted gate had `0` good-to-non-good regressions, `0` timeouts, and `0` Taxicab errors. Category movement was `corrupt_or_truncated_pdf -10`, `missing_pdf_harvest -2`, and `good_pdf +12`.

Important denominator update: the `6,293` denominator is a legacy guessed-PDF-candidate denominator, not proof that every row has a public full-text PDF. Phase A sidecars label `/Users/shubh-trips/Documents/OpenAlex/parseland-eval/eval/human-goldie.csv` as a 100-row seed; Phase B sidecars label `/Users/shubh-trips/Documents/OpenAlex/parseland-eval/eval/data/merged-FINAL.csv` for the full 10K. The draft full-corpus sidecar counts are public TRUE `2,514`, public FALSE `3,680`, REVIEW `3,806`, and all-known TRUE `2,520`. The draft public TRUE metric is now `2,455/2,514` (`97.65%`), but this is provisional and does not complete the goal until the `3,806` REVIEW rows are resolved or explicitly scoped. The new gate recovered `10` draft public TRUE rows and `2` Wiley REVIEW rows.
...
Latest oxjobs #461 report state: `6516dfdec #461 taxicab-pdf: accept public true cache gate` is pushed to `origin/main`; CI was in progress at this handoff. The report shows the raw legacy metric, denominator audit, `Where The PDF Gap Is`, the draft public TRUE failure targets, Wiley support evidence, ACS accepted targeted recovery, SAGE accepted two-row targeted recovery, J-STAGE encrypted-PDF validation evidence, and the new aggregate artifact `evidence/report461-full10k-after-publictrue-cache-summary-477d2f8.json`.

Where the next safe retrieval work is: draft public TRUE non-good rows are now `59`, not the stale pre-gate `69`. Category split is `corrupt_or_truncated_pdf=54`, `encrypted_or_unreadable_pdf=4`, and `missing_pdf_harvest=1`. Top current public TRUE hosts are `onlinelibrary.wiley.com` (19), unknown-host corrupt rows (9), `jstage.jst.go.jp` (5), and `journals.sagepub.com` (4). Wiley recovered `0/5` in no-storage provider probing and stays an Envoy-Zyte support lane. ACS is closed at targeted/full-gate level. SAGE recovered `2/7` targeted and those two are accepted in the full gate; residual SAGE remains provider-support evidence. J-STAGE recovered `0/5`; residual work is validator/provider policy, not broad route guessing.

Next exact command:

```bash
cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab && python3 scripts/pdf_availability_gold.py --input /Users/shubh-trips/Documents/OpenAlex/parseland-eval/eval/data/merged-FINAL.csv --out /tmp/merged-FINAL-pdf-availability-after-publictrue-cache.draft.csv --review-queue /tmp/merged-FINAL-pdf-review-queue-after-publictrue-cache.csv --eval-rows /tmp/taxicab-pdf-after-publictrue-cache-gate/taxicab-pdf-after-publictrue-cache-477d2f8/rows.ndjson --seed-sidecar /Users/shubh-trips/Documents/OpenAlex/parseland-eval/eval/human-goldie-pdf-availability.draft.csv --summary-json /tmp/merged-FINAL-pdf-availability-after-publictrue-cache-summary.json --public-true-failures-out /tmp/taxicab-pdf-public-true-failures-after-cache.csv --public-true-failures-summary-json /tmp/taxicab-pdf-public-true-failures-after-cache-summary.json
```
...
Latest commit/push status: Taxicab `main` is pushed at `8b36486`; Taxicab sidecar tooling branch `codex/taxicab-pdf-gold-availability` is pushed at `477d2f8`; oxjobs `main` is pushed at `6516dfdec`. This current docs slice records the accepted full gate and sets the public TRUE residual refresh as the next command.
