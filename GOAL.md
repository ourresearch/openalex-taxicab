...
Latest JPET REVIEW evidence slice: private exact-host no-storage Zyte run `jpet-review-pack-provider5-23d755e` sampled `5` `jpet.aspetjournals.org` rows from the private top-250 REVIEW pack. The probe input classified the rows as Elsevier-attributed, but the host lane is JPET/ASPET. It recovered `0/5` valid PDFs; the best aggregate category was `js_redirect_unresolved=5`, with strategy-level attempts split between unresolved JavaScript redirects and empty responses. Private overlay joined all five rows and made `0` status or denominator changes. Public aggregate asset `evidence/report461-jpet-review-provider-overlay-summary-23d755e.json` is published in oxjobs #461 at commit `7995b1d9d`; CI run `28074639145` passed and the live raw report/asset were verified after route retry. Accepted sidecar counts are unchanged until the private overlay is reviewed and adopted.
