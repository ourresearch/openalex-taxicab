...
Latest oxjobs #461 report state: <code>a1a147690 #461 taxicab-pdf: update gold denominator snapshot</code> is pushed to <code>origin/main</code>, CI run <code>28043351446</code> passed, and cache-busted raw routes verified the auth-wall denominator snapshot. Oxjobs still needs a follow-up report-only update for the <code>d468bf4</code> no-PDF-expected denominator reduction from REVIEW <code>3,152</code> to <code>3,090</code>.
...
Post-automation REVIEW concentration: <code>3,090</code> rows remain. Top hosts are <code>link.springer.com</code> (594), <code>onlinelibrary.wiley.com</code> (417), <code>degruyterbrill.com</code> (195), <code>sciencedirect.com</code> (139), <code>journals.lww.com</code> (139), <code>cambridge.org</code> (123), <code>academic.oup.com</code> (96), <code>papers.ssrn.com</code> (73), <code>jstor.org</code> (60), <code>api.taylorfrancis.com</code> (52), and <code>pubs.acs.org</code> (45). The only remaining bot-check REVIEW row has a concrete PDF URL; keep it in REVIEW until Browserbase/Zyte evidence clarifies availability.
...
After the oxjobs report-only update for <code>d468bf4</code>, choose one Goldsmith-PDF denominator-review lane by expected impact. Do not browse every DOI initially. Stop blind public TRUE tail probing unless a new provider recipe appears. The next substantive work should be denominator REVIEW reduction by top host/reason or Envoy-Zyte support packets for accumulated provider failures.

Current blocker: denominator review. The remaining public TRUE residuals are mostly exhausted provider/validator tails, not a broad Taxicab runtime failure. Continue retrieval work only for <code>pdf_gold_include_in_public_denominator=TRUE AND latest_taxicab_category != good_pdf</code>; keep public FALSE and REVIEW rows separate. The provisional public TRUE metric is above 95%, but the <code>/goal</code> is not complete while <code>3,090</code> REVIEW rows remain.

Latest verification for <code>d468bf4</code>: <code>python3 -m unittest discover -s tests</code> passed 182 tests; <code>python3 scripts/taxicab_pdf_eval.py --fixture-smoke --out /tmp/taxicab-pdf-fixture-smoke</code> passed; <code>git diff --check</code> passed; tracked-file secret/signed-query scan had no matches. Latest commit/push status before this handoff update: Taxicab sidecar branch is pushed at <code>d468bf4</code>; oxjobs report follow-up is pending.
...
Current publish status: oxjobs #461 commit `e511d26aa` records the
post-JournalUniga 0/8 singleton/tail provider probes. The report still publishes
the JournalUniga full gate, the fixed 0-origin 0-100 chart, and the updated
denominator audit. Oxjobs CI run `28041494483` passed; the live raw report and
new JSON asset were verified after retry.
Draft public TRUE is 2,463/2,513 (98.01%) but remains provisional until the
3,152 REVIEW rows are resolved.
...
`good_pdf` (39.14%), +2 versus the prior accepted gate and +626 versus the
...
accepted metric is `taxicab-pdf-after-journaluniga-cache-fc0be25`,
2,463/6,293 `good_pdf` (39.14%), with a 3,516-row gap to 95% on the legacy raw
