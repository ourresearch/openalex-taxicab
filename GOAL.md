...
Gate 21.999em: implement narrow ACM PDF-byte route candidate on branch only. [next]
...
PDF:
  ...
  latest implementation candidates: 3b2d218 taxicab: fetch Wiley pdfdirect as PDF bytes; 07c8f95 taxicab: fetch IOP article PDFs as bytes
  local candidate validation: Wiley 13/19 current residual rows classified good_pdf through local http_get; IOP article-PDF route validation `iop-http-get-local-route-precommit` returned 11/16 good_pdf and 5 bot_block_403; no Taxicab POST/R2/DynamoDB writes
