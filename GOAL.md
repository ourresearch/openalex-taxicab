...
useful Taxicab HTML rows: 85
useful Parseland rows: 85
```

Cumulative batches 001-062:
...
passes: 3,978
failures: 294
score on ready rows: 93.12%
public PDF rows: 1,575/1,581 Taxicab found real PDFs
Taxicab found real PDFs: 1,955
useful Taxicab HTML rows: 4,829
useful Parseland rows: 5,310
```

The same six public-PDF attention rows are unchanged. Batch 062 added no
...
`batch_e2e_runs/batch-001/rows.csv` through `batch_e2e_runs/batch-062/rows.csv`.
...
After batch 063, update oxjobs #461 with aggregate counts only. Do not publish
