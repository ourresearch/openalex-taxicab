...
no-storage path-family probe, or a batch of similar direct-PDF candidate lanes.
Next exact command:
...
Gate 21.999fg: choose one small fresh path-family probe or Browserbase/Zyte gold comparison. [next, no Taxicab main push]
...
PDF:
  ...
  latest oxjobs publication: 7b551e72 #461 taxicab-pdf: publish unifsa fresh tail lift
  ...
  latest bounded lift: unifsa-current-missing-provider-probe2-f6e9c80 recovered 2/2 direct PDF bytes; DOI-only reharvest recovered 0/2; direct-PDF-URL reharvest and read-only confirmation recovered 2/2 each; accepted full gate stays 2,304/6,293 until a full gate confirms the cache lift
