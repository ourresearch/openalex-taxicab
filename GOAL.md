...
public PDF rows: 1,550/1,555 Taxicab found real PDFs
Taxicab found real PDFs: 1,920
useful Taxicab HTML rows: 4,744
useful Parseland rows: 5,225
```

The same six public-PDF attention rows are unchanged. Batch 061 added no
public-PDF miss and four label mismatches where Taxicab found real PDFs even
though the sidecar says no public PDF. Local row details are in
`batch_e2e_runs/batch-001/rows.csv` through `batch_e2e_runs/batch-061/rows.csv`.
...
After batch 062, update oxjobs #461 with aggregate counts only. Do not publish
