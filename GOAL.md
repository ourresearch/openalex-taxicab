...
Latest JAMA REVIEW evidence slice: private exact-host no-storage Zyte run `jama-review-pack-provider5-a3f75fa` sampled `5` `jamanetwork.com` rows from the private top-250 REVIEW pack. It recovered `0/5` valid PDFs; best categories were `js_redirect_unresolved=3` and `interstitial_or_paywall=2`. Private overlay joined all five rows and would move `2` rows from REVIEW to public FALSE/all-known TRUE as paywall/login evidence; `3` rows stay REVIEW. Public aggregate asset `evidence/report461-jama-review-provider-overlay-summary-a3f75fa.json` is published in oxjobs #461 at commit `cd90c4699`; CI run `28071512241` passed and the live raw report/asset were verified after route retry. Accepted sidecar counts are unchanged until the private overlay is reviewed and adopted.

Recent REVIEW evidence, newest first: `asmedigitalcollection.asme.org` found two public TRUE candidates and three paywall/login candidates (`2/5`, residual `interstitial_or_paywall=3`); `nature.com` found three paywall/login candidates and two remaining REVIEW rows (`0/5`, `interstitial_or_paywall=3`, `js_redirect_unresolved=2`); `thelancet.com` stayed REVIEW (`0/5`, `empty_response=3`, `js_redirect_unresolved=2`); `iopscience.iop.org` found two public TRUE candidates and two paywall/login candidates (`2/5`, residual `interstitial_or_paywall=2`, `bot_block_403=1`); `aip.scitation.org` stayed REVIEW. Older no-movement samples remain REVIEW unless a private overlay is explicitly reviewed and adopted.
...
```bash
cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab && python3 scripts/provider_pdf_probe.py --input /Users/shubh-trips/Documents/OpenAlex/parseland-eval/eval/data/merged-FINAL-pdf-review-pack-top250.csv --category '' --host karger.com --limit 5 --strategies all --out /tmp/taxicab-pdf-review-provider-probes --run-id karger-review-pack-provider5-<taxicab-sha> --env-file .env --timeout 45 --sleep 0.5
```
