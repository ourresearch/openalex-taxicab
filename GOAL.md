...
Current publish status: oxjobs #461 commit `dc4438dc` publishes unknown direct-PDF recovery after the Nature/Oxford provider probes. Unknown IJISRT recovered 3/3, ISCA recovered 2/2, Microbiology Research recovered 1/3, and Diabetes Journals recovered 0/3 in no-storage current-provider probes at Taxicab `2244ccc`. Bounded production-write reharvest `pdf-unknown-ijisrt-isca-reharvest5-2244ccc` and read-only confirmation `pdf-unknown-ijisrt-isca-readonly5-2244ccc` both classify 5/5 as durable `good_pdf`. The same oxjobs commit sanitizes old signed URL fields from `working/taxicab-pdf/evidence/hardness-set.json`.
Current phase: unknown direct-PDF recovery is published as bounded durable cache lift, not accepted full-10K KPI lift. Latest accepted full-gate refresh remains `pdf-full10k-publisher-attribution-e584811` at Taxicab `8a35869`: 2,196/6,293 `good_pdf` (34.90%), 3,805 missing, 65 corrupt/truncated, 0 timeout, 0 `taxicab_error`, and a 3,783-row gap to 95%. Next lane is either another small unknown direct-PDF cluster or a full-gate confirmation batch after enough bounded recoveries accumulate. Do not push Taxicab main before the full PDF 95% proof.
Next exact command:
