...
Important denominator update: the <code>6,293</code> denominator is a legacy guessed-PDF-candidate denominator, not proof that every row has a public full-text PDF. Phase A sidecars label <code>/Users/shubh-trips/Documents/OpenAlex/parseland-eval/eval/human-goldie.csv</code> as a 100-row seed; regenerated counts remain public TRUE <code>12</code>, public FALSE <code>23</code>, REVIEW <code>65</code>, and all-known TRUE <code>23</code>. Phase B sidecars label <code>/Users/shubh-trips/Documents/OpenAlex/parseland-eval/eval/data/merged-FINAL.csv</code> for the full 10K; after overlays through Cambridge, counts are public TRUE <code>2,514</code>, public FALSE <code>4,407</code>, REVIEW <code>3,079</code>, and all-known TRUE <code>3,185</code>. The draft public TRUE metric is <code>2,464/2,514</code> (<code>98.01%</code>), but this is provisional and does not complete the goal until the <code>3,079</code> REVIEW rows are resolved or explicitly scoped.

Latest oxjobs #461 report state: oxjobs commit <code>9d40246c8 #461 taxicab-pdf: publish ssrn rendered delivery evidence</code> is pushed to <code>origin/main</code>, CI run <code>28052248345</code> passed, and the live raw report plus <code>report461-ssrn-review-browserbase-rendered-delivery-summary-310cb6e.json</code> were verified. The report keeps the fixed 0-100 graph, raw <code>2,464/6,293</code>, public TRUE <code>2,514</code>, REVIEW <code>3,079</code>, and records that Oxford and SSRN evidence moved <code>0</code> denominator rows.

Latest SSRN evidence: first aggregate SSRN Browserbase/rendered-delivery evidence found public-looking controls for <code>3/3</code> sampled <code>papers.ssrn.com</code> REVIEW rows but captured <code>0/3</code> valid PDF bytes. This branch then added Browserbase evidence code to click likely PDF controls and navigate discovered PDF candidate URLs in the same session. The follow-up local run <code>ssrn-review-browserbase-candidate-gold3-310cb6e</code> still captured <code>0/3</code> valid PDF bytes: Browserbase verdicts were <code>html_not_pdf=2</code> and <code>download_started_not_captured=1</code>, with one download event detected and candidate metadata present on two rows. SSRN rows remain REVIEW; this is a provider/download-capture lane, not a Taxicab route-promotion lane.
...
Current publish status: oxjobs #461 commit `f1cfd2790` records the Cambridge
bounded recovery and full 10K gate. The report publishes the Cambridge full
gate, the fixed 0-origin 0-100 chart, and the updated denominator audit.
Oxjobs CI run `28049928235` passed; the live raw report and new JSON asset were
