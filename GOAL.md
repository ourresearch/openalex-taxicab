...
Current phase: Gate 21.999do complete; Gate 21.999dp next. Structured PDF parser is implemented at Taxicab commit a61d34b and oxjobs #461 commit dcb7bb14 publishes the accepted structured-parser full gate. Oxjobs #461 commit 6ba84787 publishes `wiley-residual-corrupt-provider-probe-19-a61d34b`, which recovered 15/19 current residual Wiley corrupt rows without Taxicab POST or production writes. Taxicab commit 3b2d218 routes Wiley `/doi/pdfdirect/` URLs through PDF-byte Zyte HTTP strategies instead of browser HTML. Local no-storage `http_get` validation over the same 19 rows returned 13/19 `good_pdf`; oxjobs #461 commit d4f99eee publishes the candidate evidence. Confirmation-path decision: remote `--reharvest` tests deployed main, while local branch `Harvester` can write production R2/DynamoDB with real env credentials, so do not use local branch writes as a silent confirmation path. Oxjobs #461 commit c15cd194 publishes the follow-up probes and corrected ScienceDirect current-missing probe: Wiley `/doi/pdf/` as-is recovered 0/10, Wiley rewrite-to-`pdfdirect` recovered only 2/10 with one DOI mismatch, Springer `content/pdf` recovered 0/10, and ScienceDirect current missing recovered 0/10 after Taxicab commit 69553ae fixed host-filter normalization. Oxjobs #461 commit d8e62ef8 refreshes the combined Zyte PDF-byte support packet with that evidence. Taxicab commit e584811 classifies publishers from source `PDF URL`/candidate PDF URL fields, reducing accepted full-gate `missing_pdf_harvest` unknown-publisher rows from 966 to 642; oxjobs #461 commit 8b1d1b2f publishes the report slice. Read-only refresh `pdf-full10k-publisher-attribution-e584811` at Taxicab commit 8a35869 is 2,196/6,293 good_pdf (34.90%), with 3,805 missing, 65 corrupt/truncated, 0 timeout, 0 taxicab_error, and a 3,783-row gap to 95%; oxjobs #461 commit ebe97f4d publishes the sanitized report. Current Springer probe `springer-current-missing-provider-probe10-8a35869` at Taxicab commit 8585a77 recovered 0/10, split between 5 interstitial/paywall and 5 unresolved JS; oxjobs #461 commit 84760121 publishes the evidence. Current Wiley probe `wiley-current-missing-provider-probe10-8585a77` at Taxicab commit fa95e59 recovered 0/10, split between 9 unresolved JS and 1 bot block; all 40 strategy attempts stayed HTML, empty, or blocked, and oxjobs #461 commit 3480ae82 publishes the evidence. Current Elsevier probe `elsevier-current-missing-provider-probe10-fa95e59` at Taxicab commit 8abd909 recovered 0/10, split between 7 unresolved JS, 2 HTML-instead-of-PDF, and 1 empty response; oxjobs #461 commit 68c2eb46 publishes the evidence. Current De Gruyter probe `degruyter-current-missing-provider-probe10-eb75f5e` at Taxicab commit eb75f5e recovered 0/10, split between 8 unresolved JS and 2 bot blocks; browser HTML hit captcha for all 10, and PDF-byte header variants returned empty bodies. Oxjobs #461 commit b04396d6 publishes the evidence. Current Lippincott probe `lippincott-current-missing-provider-probe10-eb75f5e` at Taxicab commit 4689af7 recovered 0/10; all 40 strategy attempts were `download_404` against LWW `PageNotFound.aspx` HTML. Oxjobs #461 commit 40cf1b9e publishes the evidence. Current Oxford probe `oxford-current-missing-provider-probe10-4689af7` at Taxicab commit 67187dc recovered 1/10; residuals were 5 bot blocks, 3 HTML-instead-of-PDF, and 1 unresolved JS. Oxjobs #461 commit d4b6da1b publishes the evidence. Current CUP/Cambridge probe `cup-current-missing-provider-probe10-67187dc` at Taxicab commit 41a9df8 recovered 0/10; all 40 strategy attempts were `js_redirect_unresolved`. Oxjobs #461 commit 38844dea publishes the evidence. Current Taylor probe `taylor-current-missing-provider-probe10-70f8f8a` at Taxicab commit 70f8f8a recovered 0/10; best categories were 6 download_404, 3 interstitial/paywall, and 1 empty response. Oxjobs #461 commit af13892d publishes the evidence. Treat Springer, current Wiley, current Elsevier, current De Gruyter, current Lippincott, current Oxford, current CUP/Cambridge, and current Taylor residual missing clusters as provider/Zyte PDF-byte or candidate-discovery debt, not route-code candidates. Next is a bounded no-storage probe on the current SSRN missing cluster.
Next exact command:
cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab && python3 scripts/provider_pdf_probe.py --input pdf_eval_runs/pdf-full10k-publisher-attribution-e584811/rows.ndjson --category missing_pdf_harvest --publisher ssrn --limit 10 --strategies all --out pdf_eval_runs/ --run-id ssrn-current-missing-provider-probe10-70f8f8a --timeout 60 --sleep 1
```
...
Gate 21.999dp: run current SSRN missing-PDF no-storage provider probe from the refreshed full gate. [next]
...
PDF:
  ...
  latest oxjobs publication: af13892d #461 taxicab-pdf: publish current Taylor probe
  ...
  latest provider probe: taylor-current-missing-provider-probe10-70f8f8a at taxicab 70f8f8a, 0/10 recovered, best categories 6 download_404, 3 interstitial_or_paywall, and 1 empty_response; oxjobs af13892d; candidate-discovery/provider lane, not route-code candidate
