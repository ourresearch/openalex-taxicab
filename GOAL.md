...
Latest AIP Scitation REVIEW evidence slice: private exact-host no-storage Zyte run `aip-scitation-review-pack-provider5-2f76909` sampled `5` `aip.scitation.org` rows from the private top-250 REVIEW pack. It recovered `0/5` valid PDFs; best categories were `html_instead_of_pdf=3` and `js_redirect_unresolved=2`. Private overlay joined all five rows but made `0` status or denominator changes, so these rows stay REVIEW. Public aggregate asset `evidence/report461-aip-scitation-review-provider-overlay-summary-2f76909.json` is published in oxjobs #461 at commit `1d75003ae`; CI run `28068462823` passed and the live raw report/asset were verified after cache retry. Accepted sidecar counts are unchanged.

Recent REVIEW evidence, newest first: `journals.sagepub.com` stayed REVIEW (`0/5`, `js_redirect_unresolved=4`, `empty_response=1`); `journals.uchicago.edu` found one public TRUE candidate (`1/5`, residual `js_redirect_unresolved=4`); `brill.com` stayed REVIEW (`bot_block_403=5`); `opg.optica.org` found one public TRUE candidate; DOI.org, APS, RSC, Thieme, SPIE, LWW, De Gruyter/Brill, JSTOR, and other no-movement samples remain REVIEW unless a private overlay is explicitly reviewed and adopted.

Review-pack top hosts already sampled for denominator evidence include `api.taylorfrancis.com`, `jstor.org`, `pubs.acs.org`, `degruyterbrill.com`, `journals.lww.com`, `spiedigitallibrary.org`, `thieme-connect.de`, `pubs.rsc.org`, `tandfonline.com`, `pubs.aip.org`, `link.aps.org`, `doi.org`, `opg.optica.org`, `brill.com`, `journals.uchicago.edu`, `journals.sagepub.com`, and `aip.scitation.org`.

Good next unsampled review-pack hosts include `iopscience.iop.org`, `thelancet.com`, `nature.com`, `asmedigitalcollection.asme.org`, `jamanetwork.com`, `karger.com`, `sk.sagepub.com`, `pdcnet.org`, `actahort.org`, `eurekaselect.com`, and `journals.ametsoc.org`. Higher-volume hosts such as `link.springer.com`, `onlinelibrary.wiley.com`, `sciencedirect.com`, `cambridge.org`, `academic.oup.com`, and `papers.ssrn.com` have substantial prior evidence elsewhere; do not rerun them unless testing a new provider-advised recipe or a specific denominator question.
...
```bash
cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab && python3 scripts/provider_pdf_probe.py --input /Users/shubh-trips/Documents/OpenAlex/parseland-eval/eval/data/merged-FINAL-pdf-review-pack-top250.csv --category '' --host iopscience.iop.org --limit 5 --strategies all --out /tmp/taxicab-pdf-review-provider-probes --run-id iopscience-review-pack-provider5-<taxicab-sha> --env-file .env --timeout 45 --sleep 0.5
```
