...
Current phase: readable-encrypted validator lift is accepted and published. Residual clustering is complete for this full gate: 3,989 non-good rows across 174 clusters, dominated by missing PDF harvest, with a smaller actionable corrupt/truncated lane. Current corrupt no-storage probes found Wiley 9/18 recovered, ACS 6/6 recovered, Sage 0/6 recovered, and Hindawi 0/2 recovered with PDF-byte strategies. Current branch now implements a narrow ACS `pubs.acs.org/doi/pdf/...` PDF-byte route candidate only; it does not route ACS `/doi/epdf/` paths. Local no-storage branch `http_get` validation `acs-http-get-local-route-precommit-8912673` returned 6/6 `good_pdf`, with no Taxicab POST/R2/DynamoDB writes; oxjobs `82e4812f` publishes that evidence. Hindawi probe `hindawi-current-corrupt-provider-probe2-6d11e24` recovered 0/2 and is published at oxjobs `66cc6c44`, so it is support evidence only. These do not move the accepted 10K metric until a read-only/full gate confirms them. Do not push Taxicab main before the full PDF 95% proof.
Next exact command:
...
PDF:
  ...
  latest oxjobs publication: 66cc6c44 #461 taxicab-pdf: record hindawi corrupt probe
