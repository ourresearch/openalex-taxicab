...
residual clustering for PDF rows, and oxjobs commit `65411a6c` publishes the
scrubbed aggregate report. The next lane is the Taylor API host-specific
no-storage provider probe for `api.taylorfrancis.com`; it does not write
Taxicab POST/R2/DynamoDB state.
Next exact command:
cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab && python3 scripts/provider_pdf_probe.py --input pdf_eval_runs/pdf-full10k-after-readable-encrypted-f2da963/rows.ndjson --category missing_pdf_harvest --publisher taylor --host api.taylorfrancis.com --limit 10 --out pdf_eval_runs/ --run-id taylor-api-current-missing-provider-probe10-a230505 --timeout 60
```
...
Gate 21.999fc: run Taylor API host-specific no-storage provider probe. [next, no Taxicab POST/R2/DynamoDB writes]
...
PDF:
  ...
  latest oxjobs publication: 65411a6c #461 taxicab-pdf: publish candidate host clusters
  ...
  latest provider probe: unknown-mjle-current-corrupt-provider-probe1-60ac8e3 at taxicab add6ef1 recovered 0/1; candidate-host residual clustering at taxicab a230505 is the current handoff and points next to the Taylor API host-specific provider probe on api.taylorfrancis.com
