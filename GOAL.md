...
Current publish status: oxjobs #461 commit `eb59cb586` publishes the SJNS provider/gold negative evidence, residual demotion `residual-clusters-after-sjns-531101a`, graph/report update, learning notes, improvement plan, and next `sjdem.sljol.info` handoff. CI run `27785689884` passed and the live raw report plus SJNS summary JSON were verified with a cache-busting request.
...
scraping push. After the Solen full gate, the residual refresh has 1,061
provider-lane/do-not-duplicate subclusters and 306 one-row `probe_next`
subclusters. The next fresh singleton probe is `sjns.journals.ekb.eg`.
Current phase: publish Solen accepted recovery to #461, then run the
`sjns.journals.ekb.eg` no-storage evidence probe before any Taxicab main push.
Do not
