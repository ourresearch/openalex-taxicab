...
passes: 61
failures: 3
score on ready rows: 95.31%
public PDF rows: 28/28 Taxicab found real PDFs
Taxicab found real PDFs: 34
useful Parseland rows: 91
```

Cumulative batches 001-030:
...
public PDF rows: 760/763 Taxicab found real PDFs
Taxicab found real PDFs: 952
useful Parseland rows: 2,552
```
...
revistas.filos.unam.mx tiny/corrupt PDF-like response. Batch 030 added no new
public-PDF miss. Its three failures are likely label fixes: the sidecar said no
...
`batch_e2e_runs/batch-030/rows.csv`. Public oxjobs gets aggregate counts only.
...
After batch 031, update oxjobs #461 with aggregate counts only. Do not publish
