...
Latest ASME REVIEW evidence slice: private exact-host no-storage Zyte run `asme-review-pack-provider5-08db747` sampled `5` `asmedigitalcollection.asme.org` rows from the private top-250 REVIEW pack. It recovered valid PDFs for `2/5` rows; best residual category was `interstitial_or_paywall=3`. Private overlay joined all five rows and would move `2` rows from REVIEW to public TRUE/all-known TRUE and `3` rows from REVIEW to public FALSE/all-known TRUE as paywall/login evidence. Public aggregate asset `evidence/report461-asme-review-provider-overlay-summary-08db747.json` is published in oxjobs #461 at commit `ac4c0c74f`; CI run `28070775090` passed and the live raw report/asset were verified after route retry. Accepted sidecar counts are unchanged until the private overlay is reviewed and adopted.

Recent REVIEW evidence, newest first: `nature.com` found three paywall/login candidates and two remaining REVIEW rows (`0/5`, `interstitial_or_paywall=3`, `js_redirect_unresolved=2`); `thelancet.com` stayed REVIEW (`0/5`, `empty_response=3`, `js_redirect_unresolved=2`); `iopscience.iop.org` recovered `2/5` valid PDFs and produced private overlay movements for review; `aip.scitation.org` stayed REVIEW (`0/5`, `html_instead_of_pdf=3`, `js_redirect_unresolved=2`); `journals.sagepub.com` stayed REVIEW; `journals.uchicago.edu` found one public TRUE candidate. Older no-movement samples remain REVIEW unless a private overlay is explicitly reviewed and adopted.

Review-pack top hosts already sampled for denominator evidence include `api.taylorfrancis.com`, `jstor.org`, `pubs.acs.org`, `degruyterbrill.com`, `journals.lww.com`, `spiedigitallibrary.org`, `thieme-connect.de`, `pubs.rsc.org`, `tandfonline.com`, `pubs.aip.org`, `link.aps.org`, `doi.org`, `opg.optica.org`, `brill.com`, `journals.uchicago.edu`, `journals.sagepub.com`, `aip.scitation.org`, `iopscience.iop.org`, `thelancet.com`, `nature.com`, and `asmedigitalcollection.asme.org`.

Good next unsampled review-pack hosts include `jamanetwork.com`, `karger.com`, `sk.sagepub.com`, `pdcnet.org`, `actahort.org`, `eurekaselect.com`, and `journals.ametsoc.org`. Higher-volume hosts such as `link.springer.com`, `onlinelibrary.wiley.com`, `sciencedirect.com`, `cambridge.org`, `academic.oup.com`, and `papers.ssrn.com` have substantial prior evidence elsewhere; do not rerun them unless testing a new provider-advised recipe or a specific denominator question.
...
```bash
cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab && python3 scripts/provider_pdf_probe.py --input /Users/shubh-trips/Documents/OpenAlex/parseland-eval/eval/data/merged-FINAL-pdf-review-pack-top250.csv --category '' --host jamanetwork.com --limit 5 --strategies all --out /tmp/taxicab-pdf-review-provider-probes --run-id jama-review-pack-provider5-<taxicab-sha> --env-file .env --timeout 45 --sleep 0.5
```
