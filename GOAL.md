...
Current publish status: oxjobs #461 commit `d9455b42` publishes the current unknown-provider refresh. Taxicab `8cea72c` probed 12 current unknown-host clusters with no Taxicab storage writes, then ran bounded reharvest/read-only confirmations for the recoverable Chicago and Physiology samples. Chicago preserved 1/5 durable `good_pdf`; Physiology preserved 2/5 durable `good_pdf`; Inlibra, Scientific.Net, Persee, PDCNet, ASCE, ActaHort, IWA, Healio, UC Press, and AMPP recovered 0/5 each in the current-provider no-storage probes.
Current phase: unknown-provider refresh is published as bounded durable cache lift, not accepted full-10K KPI lift. Latest accepted full-gate refresh remains `pdf-full10k-publisher-attribution-e584811` at Taxicab `8a35869`: 2,196/6,293 `good_pdf` (34.90%), 3,805 missing, 65 corrupt/truncated, 0 timeout, 0 `taxicab_error`, and a 3,783-row gap to 95%. Bounded post-refresh recoveries since that full gate are +9 total: IJISRT/ISCA +5, AJOL +1, Chicago/Physiology +3. Next lane is the full 10K read-only confirmation gate after unknown-provider refresh. Do not push Taxicab main before the full PDF 95% proof.
Next exact command:
