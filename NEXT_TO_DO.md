...
Current gate: structured PDF parser is implemented at Taxicab commit `a61d34b`; oxjobs #461 is pushed at `dcb7bb14` with the accepted structured-parser full gate, at `9569e1f6` with the Wiley residual 5-row probe, and at `6ba84787` with the Wiley residual all-row probe. Taxicab commit `3b2d218` implements the narrow Wiley PDF-byte strategy candidate. Oxjobs #461 commit `d4f99eee` publishes that candidate evidence. Taxicab commit `69553ae` fixes provider-probe host-filter normalization, oxjobs #461 commit `c15cd194` publishes the Wiley/Springer/ScienceDirect follow-up probes and no-route-code decision, and oxjobs #461 commit `d8e62ef8` refreshes the combined Zyte PDF-byte support packet. Taxicab commit `e584811` classifies publishers from source `PDF URL`/candidate PDF URL hosts, reducing accepted full-gate `missing_pdf_harvest` unknown-publisher rows from 966 to 642; oxjobs #461 commit `8b1d1b2f` publishes this triage slice. Current read-only refresh `pdf-full10k-publisher-attribution-e584811` at Taxicab commit `8a35869` is 2,196/6,293 `good_pdf` (34.90%), with 3,805 missing, 65 corrupt/truncated, 0 timeout, 0 `taxicab_error`, and a 3,783-row gap to 95%; oxjobs #461 commit `ebe97f4d` publishes the sanitized full-gate report. Current Springer no-storage probe `springer-current-missing-provider-probe10-8a35869` at Taxicab commit `8585a77` recovered 0/10 and is published at oxjobs #461 commit `84760121`; current Wiley no-storage probe `wiley-current-missing-provider-probe10-8585a77` at Taxicab commit `fa95e59` recovered 0/10 and is published at oxjobs #461 commit `3480ae82`; current Elsevier no-storage probe `elsevier-current-missing-provider-probe10-fa95e59` at Taxicab commit `8abd909` recovered 0/10 and is published at oxjobs #461 commit `68c2eb46`; current De Gruyter no-storage probe `degruyter-current-missing-provider-probe10-eb75f5e` at Taxicab commit `eb75f5e` recovered 0/10 and is published at oxjobs #461 commit `b04396d6`; current Lippincott no-storage probe `lippincott-current-missing-provider-probe10-eb75f5e` at Taxicab commit `4689af7` recovered 0/10 and is published at oxjobs #461 commit `40cf1b9e`; current Oxford no-storage probe `oxford-current-missing-provider-probe10-4689af7` at Taxicab commit `67187dc` recovered 1/10 and is published at oxjobs #461 commit `d4b6da1b`; current CUP/Cambridge no-storage probe `cup-current-missing-provider-probe10-67187dc` at Taxicab commit `41a9df8` recovered 0/10 and is published at oxjobs #461 commit `38844dea`; current Taylor no-storage probe `taylor-current-missing-provider-probe10-70f8f8a` at Taxicab commit `70f8f8a` recovered 0/10 and is published at oxjobs #461 commit `af13892d`; current SSRN no-storage probe `ssrn-current-missing-provider-probe10-863d7aa` at Taxicab commit `863d7aa` recovered 0/10 with all best categories `js_redirect_unresolved` and is published at oxjobs #461 commit `2c171c7e`; current JSTOR no-storage probe `jstor-current-missing-provider-probe10-fe0d018` at Taxicab commit `fe0d018` recovered 0/10 with all best categories `html_instead_of_pdf` and is published at oxjobs #461 commit `463bb712`. Keep Springer, current Wiley, current Elsevier, current De Gruyter, current Lippincott, current Oxford, current CUP/Cambridge, current Taylor, current SSRN, and current JSTOR residual missing rows in the provider/Zyte PDF-byte, click-download, or candidate-discovery lane. Next is a bounded no-storage probe on the current AIP missing cluster.
...
cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab
git switch codex/taxicab-pdf-phase2
python3 scripts/provider_pdf_probe.py --input pdf_eval_runs/pdf-full10k-publisher-attribution-e584811/rows.ndjson --category missing_pdf_harvest --publisher aip --limit 10 --strategies all --out pdf_eval_runs/ --run-id aip-current-missing-provider-probe10-fe0d018 --timeout 60 --sleep 1
```
