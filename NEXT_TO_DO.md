...
rows. IOP probe `iop-current-missing-provider-probe10-8aaf717` recovered 6/10 current missing rows as `good_pdf`; residuals were two `bot_block_403` rows and two `interstitial_or_paywall` book-chapter rows. All-current IOP confirmation `iop-current-missing-provider-probe25-ad609f7` at Taxicab `ad609f7` recovered 9/18 as `good_pdf`; residuals were six `bot_block_403`, two `interstitial_or_paywall`, and one `html_instead_of_pdf`. Oxjobs `51b4665a` publishes it. Next is narrow branch-only IOP article-PDF route code for `iopscience.iop.org/article/.../pdf` only.
