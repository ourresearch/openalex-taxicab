...
run output: batch_e2e_runs/batch-017/
total rows: 100
ready rows: 69
review rows: 31
passes: 59
...
Cumulative batches 001-017: 1,700 checked, 1,166 ready rows, 534 review rows,
1,079 passes, 87 failures, 92.54% on ready rows, 438/439 public-PDF rows
retrieved by Taxicab, and 1,458 rows with useful Parseland output.

The only public-PDF miss remains the ScienceDirect/JMRT PDF-byte retrieval
issue from batch 006. The other 86 failures are likely label fixes: Taxicab
...
`batch_e2e_runs/batch-017/rows.csv`
