...
`3f09cbf8` publishes aggregate-only priority evidence plus the first three
...
bounded no-storage path-family probe, likely `pubs.asha.org`, `pm-research.com`,
`maps.mla.org`, `journalijar.com`, or `jmcc-online.com` unless a newer full gate
supersedes the queue. Summary: 113 provider lanes; 8 fresh probes. Gate note:
