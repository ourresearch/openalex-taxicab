...
rows. IOP probe `iop-current-missing-provider-probe10-8aaf717` recovered 6/10 current missing rows as `good_pdf`; residuals were two `bot_block_403` rows and two `interstitial_or_paywall` book-chapter rows. All-current IOP confirmation `iop-current-missing-provider-probe25-ad609f7` at Taxicab `ad609f7` recovered 9/18 as `good_pdf`; residuals were six `bot_block_403`, two `interstitial_or_paywall`, and one `html_instead_of_pdf`. Oxjobs `51b4665a` publishes it. Taxicab commit `07c8f95` implements narrow branch-only IOP article-PDF route code for `iopscience.iop.org/article/.../pdf` only. Local no-storage branch `http_get` validation `iop-http-get-local-route-precommit` returned 11/16 `good_pdf` and five `bot_block_403`, with no Taxicab POST/R2/DynamoDB writes. Oxjobs `c3c9b0ac` publishes that route-validation evidence. BMJ larger confirmation recovered only 1/25 and is published at oxjobs `e5b648f6`. RSC larger confirmation recovered 0/25 and is published at oxjobs `84f1c8ea`. APS larger confirmation recovered 0/24 and is published at oxjobs `5435e2c7`. ACM larger confirmation recovered 6/22 and is published at oxjobs `88b3d53f`. Current branch ACM `/doi/pdf/` route validation `acm-http-get-local-route-precommit-1950532` returned 5/22 classifier `good_pdf`, 4/22 strict URL-match recoveries, one candidate-DOI mismatch, and no Taxicab POST/R2/DynamoDB writes. ACM route regression checks, branch commit/push, and oxjobs publication are complete at Taxicab `39fa9c2` and oxjobs `695fb51d`. Next is provider-support packet refresh or the next current missing-provider cluster; do not push Taxicab main before the full PDF 95% proof.
...
Latest current-provider confirmations published in oxjobs `d9455b42`: the
unknown-provider refresh at Taxicab `8cea72c` probed 12 current unknown-host
clusters with no Taxicab storage writes. In no-storage probes, Chicago recovered
1/5 and Physiology recovered 2/5; Inlibra, Scientific.Net, Persee, PDCNet, ASCE,
ActaHort, IWA, Healio, UC Press, and AMPP recovered 0/5 each. Bounded
production-write reharvest plus read-only confirmation preserved Chicago 1/5
and Physiology 2/5 as durable `good_pdf`. Together with the earlier IJISRT/ISCA
+5 and AJOL +1 bounded durable recoveries, this makes +9 bounded cache lift
since the accepted `pdf-full10k-publisher-attribution-e584811` full gate. Treat
these as bounded durable cache lift, not accepted full-10K KPI lift until the
next full read-only gate confirms movement.
...
Current tooling slice: generic no-storage provider probing in `scripts/provider_pdf_probe.py`. It reads full-gate rows or CSV queues, sanitizes URLs, tests named Zyte strategies, and writes local probe artifacts without Taxicab POST/R2/DynamoDB writes. Provider probe summaries now choose the best non-good category per DOI instead of defaulting to the first attempted strategy; this is measurement/reporting-only.
...
J-STAGE missing-provider probe `jstage-missing-provider-probe-3-31663bc` recovered 0/3 residual missing PDFs: two rows stayed `js_redirect_unresolved`, and one row timed out empty/browser-shell. J-STAGE residuals now move to provider/support or validator-byte triage. Structured parser gate `pdf-full10k-after-structured-parser-a61d34b` is now accepted at 2,193/6,293 `good_pdf` (34.85%). Wiley residual all-row provider probe `wiley-residual-corrupt-provider-probe-19-a61d34b` recovered 15/19 current residual Wiley corrupt rows as `good_pdf`, with two `empty_response`, one `bot_block_403`, and one `supplement_or_preview_pdf` residual. Taxicab commit `3b2d218` implements Wiley PDF-byte fetching for `/doi/pdfdirect/` URLs and local no-storage `http_get` validation returned 13/19 `good_pdf`; Taxicab commit `69553ae` fixes host-filter normalization. Oxjobs #461 commit `c15cd194` publishes the follow-up weak probes and no-route-code decision: Wiley `/doi/pdf/` 0/10, Wiley rewrite 2/10 with one DOI mismatch, Springer 0/10, ScienceDirect current missing 0/10. Oxjobs #461 commit `d8e62ef8` refreshes the combined Zyte PDF-byte support packet. Taxicab commit `e584811` and oxjobs commit `8b1d1b2f` publish source-PDF-url publisher attribution. Oxjobs commit `ebe97f4d` publishes the refreshed current full-gate measurement: 2,196/6,293 `good_pdf` (34.90%), 3,805 missing, 0 timeout, 0 `taxicab_error`. Current Springer probe at Taxicab commit `8585a77` recovered 0/10 and is published at oxjobs commit `84760121`. Current Wiley probe at Taxicab commit `fa95e59` recovered 0/10 with 9 unresolved JS rows and 1 bot block; oxjobs commit `3480ae82` publishes it. Current Elsevier probe at Taxicab commit `8abd909` recovered 0/10 with 7 unresolved JS, 2 HTML-instead-of-PDF, and 1 empty response; oxjobs commit `68c2eb46` publishes it. Current De Gruyter probe at Taxicab commit `eb75f5e` recovered 0/10 with 8 unresolved JS and 2 bot blocks; oxjobs commit `b04396d6` publishes it. Current Lippincott probe at Taxicab commit `4689af7` recovered 0/10 with 10 `download_404`; oxjobs commit `40cf1b9e` publishes it. Current Oxford probe at Taxicab commit `67187dc` recovered 1/10 with 5 bot blocks, 3 HTML-instead-of-PDF, and 1 unresolved JS residual; oxjobs commit `d4b6da1b` publishes it. Current CUP/Cambridge probe at Taxicab commit `41a9df8` recovered 0/10 with all 40 strategy attempts as JS redirects; oxjobs commit `38844dea` publishes it. Current Taylor probe at Taxicab commit `70f8f8a` recovered 0/10 with 6 download_404, 3 interstitial_or_paywall, and 1 empty_response; oxjobs commit `af13892d` publishes it. Current SSRN probe at Taxicab commit `863d7aa` recovered 0/10 with all best categories `js_redirect_unresolved`; oxjobs commit `2c171c7e` publishes it. Current JSTOR probe at Taxicab commit `fe0d018` recovered 0/10 with all best categories `html_instead_of_pdf`; oxjobs commit `463bb712` publishes it. Current AIP Publishing probe at Taxicab commit `af746d4` recovered 0/10 with 6 JS redirects, 3 interstitial/paywall rows, and 1 HTML-instead-of-PDF; oxjobs commit `14f254ac` publishes it. Current RSC probe at Taxicab commit `f709792` recovered 1/10 through `google_referer`; residuals were 8 JS redirects and 1 HTML-instead-of-PDF; oxjobs commit `e3621c28` publishes it. Current ACS probe at Taxicab commit `7e4b5e5` recovered 0/10 with all best categories `js_redirect_unresolved`; oxjobs commit `21a7697c` publishes it. Current Brill probe at Taxicab commit `88bc77f` recovered 0/10 with all best categories and all 40 strategy rows `bot_block_403`; oxjobs commit `e2bac29b` publishes it. Current Thieme probe at Taxicab commit `3f45c6a` recovered 0/10 with all best categories and all 40 strategy rows `js_redirect_unresolved`; oxjobs commit `4838bd1c` publishes it. Current SPIE probe at Taxicab commit `9fa4596` recovered 0/10 with all best categories and all 40 strategy rows `js_redirect_unresolved`; oxjobs commit `fe048cca` publishes it. Current BMJ probe at Taxicab commit `9ccfeaf` recovered 2/10 with 6 bot blocks and 2 interstitial/paywall residuals; oxjobs commit `6de28ec3` publishes it. Current Sage probe at Taxicab commit `1f57c9b` recovered 0/10 with 6 bot blocks and 4 JS redirects; oxjobs commit `d059488d` publishes it. Current AMA/JAMA probe at Taxicab commit `2198bc2` recovered 0/10 with 5 interstitial/paywall rows, 4 JS redirects, and 1 bot block; oxjobs commit `eddf9c5a` publishes it. Current Karger probe at Taxicab commit `4427b24` recovered 0/10 with every best category `html_instead_of_pdf`; oxjobs commit `69b2780a` publishes it. Current APS probe at Taxicab commit `cf3d845` recovered 1/10 through `google_referer`, with nine residual `js_redirect_unresolved` rows; oxjobs commit `5da73adb` publishes it. Current ACM probe at Taxicab commit `dba7e2f` first recovered 1/10 through `default_body`/`google_referer`; larger confirmation `acm-current-missing-provider-probe25-26f35ea` at Taxicab commit `26f35ea` recovered 6/22 through `default_body`/`google_referer`, with residuals of 12 `html_instead_of_pdf`, three `js_redirect_unresolved`, and one `empty_response`; oxjobs commit `88b3d53f` publishes it and makes ACM a narrow route-strategy candidate. Current Optica probe at Taxicab commit `1b0823d` recovered 0/10, with eight `js_redirect_unresolved` rows and two `html_instead_of_pdf` rows; oxjobs commit `f57bad44` publishes it. Current IOP probe at Taxicab commit `8aaf717` recovered 6/10 as `good_pdf`, with two `bot_block_403` and two `interstitial_or_paywall` residuals; oxjobs commit `bd7396fb` publishes it. All-current IOP confirmation recovered 9/18 at Taxicab commit `ad609f7`; oxjobs commit `51b4665a` publishes it. IOP route validation for Taxicab commit `07c8f95` recovered 11/16 with no storage writes; oxjobs commit `c3c9b0ac` publishes it. BMJ larger confirmation `bmj-current-missing-provider-probe25-622512f` recovered 1/25 and oxjobs commit `e5b648f6` publishes it. RSC larger confirmation `rsc-current-missing-provider-probe25-05b1a38` recovered 0/25 and oxjobs commit `84f1c8ea` publishes it. APS larger confirmation `aps-current-missing-provider-probe25-576c058` recovered 0/24 and oxjobs commit `5435e2c7` publishes it. ACM larger confirmation `acm-current-missing-provider-probe25-26f35ea` recovered 6/22 and oxjobs commit `88b3d53f` publishes it. Unknown direct-PDF probes, AJOL tail recovery, and the current unknown-provider refresh are published through oxjobs `d9455b42`: IJISRT+ISCA durable reharvest/read-only confirmation preserved 5/5, AJOL preserved 1/3, Chicago preserved 1/5, Physiology preserved 2/5, and the current Inlibra/Scientific.Net/Persee/PDCNet/ASCE/ActaHort/IWA/Healio/UC Press/AMPP probes recovered 0/5 each. Next exact command: cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab && python3 scripts/secret_scan.py && git diff --check -- AGENTS.md CLAUDE.md GOAL.md NEXT_TO_DO.md && python3 -m unittest discover -s tests && python3 scripts/taxicab_pdf_eval.py --fixture-smoke --out /tmp/taxicab-pdf-fixture-smoke-unknown-refresh-sync --run-id pdf-fixture-smoke-unknown-refresh-sync
```
...
cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab
git switch codex/taxicab-pdf-phase2
cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab && python3 scripts/secret_scan.py && git diff --check -- AGENTS.md CLAUDE.md GOAL.md NEXT_TO_DO.md && python3 -m unittest discover -s tests && python3 scripts/taxicab_pdf_eval.py --fixture-smoke --out /tmp/taxicab-pdf-fixture-smoke-unknown-refresh-sync --run-id pdf-fixture-smoke-unknown-refresh-sync
```
