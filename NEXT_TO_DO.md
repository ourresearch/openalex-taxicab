...
Latest full-gate confirmation published in oxjobs `eacb1a53`: read-only run
`pdf-full10k-after-unknown-refresh-3d04a80` accepted 2,205/6,293 `good_pdf`
(35.04%), confirming the bounded unknown-provider cache lift. This is +9 versus
the publisher-attribution refresh, +12 versus the structured-parser gate, and
+368 versus denominator baseline. The run has 3,796 `missing_pdf_harvest`,
65 corrupt/truncated, 104 encrypted/unreadable, 92 supplement/preview,
0 timeout, and 0 `taxicab_error`. The 95% gap is 3,774 rows.
Current local validator/tooling slice: residual clustering is now PDF-aware and
excludes `good_pdf` plus `no_pdf_expected` rows from the residual queue.
Readable encrypted PDFs now count as `good_pdf` only when they have EOF,
nonzero pages, and at least 500 extracted text characters. Targeted read-only
run `pdf-jstage-readable-encrypted-target-cce66ad` on 85 J-STAGE encrypted rows
returned 80 `good_pdf`, 4 tiny encrypted residuals, and 1 supplement/preview.
This is measurement/validator-only until the next full 10K read-only gate
confirms corpus-level movement.
