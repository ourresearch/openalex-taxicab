...
RSC `e3621c28`, ACS `21a7697c`, Brill `e2bac29b`, Thieme `4838bd1c`, SPIE
`fe048cca`, BMJ `6de28ec3`, Sage `d059488d`, AMA/JAMA `eddf9c5a`, Karger
`69b2780a`, APS `5da73adb`, ACM `88c2fddb`, Optica `f57bad44`, IOP
...
recovered 1/10 through `google_referer`; residual best categories were 8
`js_redirect_unresolved` and 1 `html_instead_of_pdf`, with browser HTML hitting
6 `bot_block_403` rows. ACS probe `acs-current-missing-provider-probe10-7e4b5e5`
...
cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab
git switch codex/taxicab-pdf-phase2
RUN_SHA=$(git rev-parse --short HEAD) && python3 scripts/provider_pdf_probe.py --input pdf_eval_runs/pdf-full10k-publisher-attribution-e584811/rows.ndjson --category missing_pdf_harvest --publisher rsc --limit 25 --strategies all --out pdf_eval_runs/ --run-id rsc-current-missing-provider-probe25-$RUN_SHA --timeout 60 --sleep 1
```
