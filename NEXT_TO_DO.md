...
Latest current-provider confirmations published in oxjobs `5abc1635`: Nature
residual probe `nature-current-missing-provider-probe25-7189521` recovered
0/15, with 11 `interstitial_or_paywall` and four `js_redirect_unresolved`
best categories. Oxford larger probe
`oxford-current-missing-provider-probe25-7189521` recovered 1/25 only through a
`sciengine.com` candidate; `academic.oup.com` recovered 0/23 with 15 bot
blocks, four HTML rows, three interstitial/paywall rows, and one 404. These are
provider/Zyte support lanes, not route-code candidates.
