...
Current handoff override: Taxicab commit `a230505` implements candidate-host
residual clustering and oxjobs commit `65411a6c` publishes the scrubbed
aggregate. The next exact command is the Taylor API host-specific no-storage
provider probe for `api.taylorfrancis.com`; it does not write Taxicab
POST/R2/DynamoDB state.
...
Current tooling slice: generic no-storage provider probing in `scripts/provider_pdf_probe.py`. It reads full-gate rows or CSV queues, sanitizes URLs, tests named Zyte strategies, and writes local probe artifacts without Taxicab POST/R2/DynamoDB writes. Provider probe summaries now choose the best non-good category per DOI instead of defaulting to the first attempted strategy; this is measurement/reporting-only. Next exact command: `cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab && python3 scripts/provider_pdf_probe.py --input pdf_eval_runs/pdf-full10k-after-readable-encrypted-f2da963/rows.ndjson --category missing_pdf_harvest --publisher taylor --host api.taylorfrancis.com --limit 10 --out pdf_eval_runs/ --run-id taylor-api-current-missing-provider-probe10-a230505 --timeout 60`.
...
cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab
git switch codex/taxicab-pdf-phase2
python3 scripts/provider_pdf_probe.py --input pdf_eval_runs/pdf-full10k-after-readable-encrypted-f2da963/rows.ndjson --category missing_pdf_harvest --publisher taylor --host api.taylorfrancis.com --limit 10 --out pdf_eval_runs/ --run-id taylor-api-current-missing-provider-probe10-a230505 --timeout 60
```
