...
Latest current-provider confirmations published in oxjobs `bbf1da67`: unknown
direct-PDF probes and tail probes found bounded recoveries in small direct-PDF
clusters. IJISRT recovered 3/3 and ISCA recovered 2/2 at Taxicab `2244ccc`;
bounded IJISRT+ISCA reharvest/read-only confirmation preserved 5/5 durable
`good_pdf`. AJOL recovered 1/3 at Taxicab `484606b`; bounded reharvest
`pdf-unknown-ajol-reharvest3-484606b` and read-only confirmation
`pdf-unknown-ajol-readonly3-484606b` preserved 1/3 durable `good_pdf`.
Microbiology Research was 1/3 no-storage only; Diabetes Journals 0/3, IET 0/3,
SIAM 0/2, JoVE 0/2, AIMS 0/2, and KCI 0/2 remain provider/candidate debt.
Treat these as bounded durable cache lift, not accepted full-10K KPI lift until
a later full gate confirms movement.
...
cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab
git switch codex/taxicab-pdf-phase2
cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab && python3 scripts/secret_scan.py && git diff --check -- AGENTS.md CLAUDE.md GOAL.md NEXT_TO_DO.md && python3 -m unittest discover -s tests && python3 scripts/taxicab_pdf_eval.py --fixture-smoke --out /tmp/taxicab-pdf-fixture-smoke-ajol-tail-sync --run-id pdf-fixture-smoke-ajol-tail-sync
```
