...
Latest current-provider confirmations published in oxjobs `dc4438dc`: unknown
direct-PDF probes at Taxicab `2244ccc` found bounded recoveries in small
direct-PDF clusters. IJISRT recovered 3/3, ISCA recovered 2/2, Microbiology
Research recovered 1/3, and Diabetes Journals recovered 0/3 in no-storage
current-provider probes. Bounded reharvest
`pdf-unknown-ijisrt-isca-reharvest5-2244ccc` recovered 5/5 across IJISRT and
ISCA, and read-only confirmation `pdf-unknown-ijisrt-isca-readonly5-2244ccc`
preserved 5/5 durable `good_pdf`. Treat this as bounded durable cache lift, not
accepted full-10K KPI lift until a later full gate confirms movement. The same
oxjobs commit sanitizes old signed URL fields from the #461 hardness set.
...
cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab
git switch codex/taxicab-pdf-phase2
cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab && python3 scripts/secret_scan.py && git diff --check -- AGENTS.md CLAUDE.md GOAL.md NEXT_TO_DO.md && python3 -m unittest discover -s tests && python3 scripts/taxicab_pdf_eval.py --fixture-smoke --out /tmp/taxicab-pdf-fixture-smoke-unknown-direct-sync --run-id pdf-fixture-smoke-unknown-direct-sync
```
