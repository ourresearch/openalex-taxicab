...
Current tooling slice: generic no-storage provider probing in `scripts/provider_pdf_probe.py`. It reads full-gate rows or CSV queues, sanitizes URLs, tests named Zyte strategies, and writes local probe artifacts without Taxicab POST/R2/DynamoDB writes. Provider probe summaries now choose the best non-good category per DOI instead of defaulting to the first attempted strategy; this is measurement/reporting-only. Next exact command: `cd /Users/shubh-trips/Documents/OpenAlex/openalex-taxicab && rg -n "source_pdf|source.*url|candidate|PDF URL|host" scripts openalex_taxicab tests`.
...
python3 -m unittest discover -s tests
rg -n "source_pdf|source.*url|candidate|PDF URL|host" scripts openalex_taxicab tests
```
